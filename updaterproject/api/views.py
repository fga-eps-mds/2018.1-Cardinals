...
class RepositoryPullRequests(APIView):
    ...
    def getAll(self, repo_address):
        ...
        if repo:
            pulls = PullRequest.objects.filter(repository__full_name__contains=repo.full_name) 

            custom_data = {
                'Repository': RepositorySerializer(repo).data,
                'PullRequests': PullRequestSerializer(pulls, many=True).data
            }
